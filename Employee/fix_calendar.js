...
const pagePath = path.join(__dirname, 'src/app/(portal)/calendar/page.tsx');
...
interface CalendarEvent {
  ...
  start_at: string;
  end_at: string;
  event_type: string; // holiday, leave, meeting, event, birthday, anniversary
  all_day: boolean;
}
...
export default function CalendarPage() {
  ...
  const fetchEvents = async () => {
    ...
    let allEvents = [];
    if (calEvents) {
      allEvents = [...calEvents.map(e => ({...e, event_type: e.event_type || 'events'}))];
    }
  ...
  const filteredEvents = events.filter(e => {
    if (e.event_type === 'holiday' && !filters.holidays) return false;
    if (e.event_type === 'my_leave' && !filters.my_leave) return false;
    if (e.event_type === 'meeting' && !filters.meetings) return false;
    if (e.event_type === 'event' && !filters.events) return false;
    if (e.event_type === 'birthday' && !filters.birthdays) return false;
    if (e.event_type === 'anniversary' && !filters.anniversaries) return false;
    return true;
  });
...
console.log('Patched calendar page.');
