...
export function RejectLeaveModal({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
        <div className="flex justify-between items-center p-5 border-b border-gray-100">
          <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2"><XCircle size={20} className="text-rose-600"/> Reject Leave Request?</h2>
        ...
        <div className="p-5 space-y-4">
          <div>
            ...
            >
              ...
              <option>Insufficient Notice</option>
              <option>Missing Required Document</option>
              <option>Policy Requirement Not Met</option>
              <option>Other</option>
            </select>
          </div>
        ...
        <div className="p-5 border-t border-gray-100 flex justify-end gap-3 bg-gray-50">
          ...
          <button 
            onClick={onClose} 
            disabled={!reason}
            className={`px-4 py-2 text-sm font-medium text-white rounded-lg transition-colors shadow-sm ${reason ? 'bg-rose-600 hover:bg-rose-700' : 'bg-rose-300 cursor-not-allowed'}`}
          >
            Reject Leave
          </button>
        </div>
