...
import { Building2, Plus, Users, ShieldAlert, Award, ChevronRight, Edit3, Trash2 } from 'lucide-react';

export const DepartmentsPage = () => {
  const { departments, designations, employees, addDepartment, addDesignation } = useHR();
  ...
  const [desigForm, setDesigForm] = useState({ name: '', departmentId: departments[0]?.id || '', level: 'Mid' });

  const handleAddDept = (e) => {
    e.preventDefault();
    if (!deptForm.name || !deptForm.code) return;
    const headObj = employees.find(e => e.id === deptForm.headEmployeeId);
    addDepartment({
      ...deptForm,
  ...
  const handleAddDesig = (e) => {
    ...
    const deptObj = departments.find(d => d.id === desigForm.departmentId);
    addDesignation({
      ...desigForm,
      departmentName: deptObj ? deptObj.name : 'General'
    });
    setIsAddDesigModal(false);
    setDesigForm({ name: '', departmentId: departments[0]?.id || '', level: 'Mid' });
  };
    ...
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => setIsAddDesigModal(true)}
            ...
            return (
              <div key={dept.id} className="p-5 rounded-3xl bg-white border border-slate-200/80 shadow-sm hover:shadow-md transition-all flex flex-col justify-between">
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-slate-400 font-medium">Est. 2025</span>
                </div>
              </div>
            );
          })}
          ...
          <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl border border-slate-100">
            ...
            <form onSubmit={handleAddDept} className="mt-4 space-y-3 text-xs">
              ...
              <div className="flex justify-end gap-2 pt-3">
                ...
                <button type="submit" className="px-4 py-2 rounded-xl bg-saath-600 text-white font-bold">Create Department</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl border border-slate-100">
            ...
            <form onSubmit={handleAddDesig} className="mt-4 space-y-3 text-xs">
              ...
              <div className="flex justify-end gap-2 pt-3">
                ...
                <button type="submit" className="px-4 py-2 rounded-xl bg-saath-600 text-white font-bold">Create Designation</button>
              </div>
            </form>
          </div>
        </div>
