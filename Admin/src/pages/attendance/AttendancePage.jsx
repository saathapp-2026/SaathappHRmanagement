...
export const AttendancePage = () => {
  ...
  const todayRecords = dailyAttendance.filter(a => a.date === todayStr || a.attendance_date === todayStr);
  ...
  const holidayCount = todayRecords.filter(a => a.status === 'Holiday').length;
  const weeklyOffCount = todayRecords.filter(a => a.status === 'Weekly Off').length;
  const wfhCount = todayRecords.filter(a => a.status === 'Work From Home' || a.location === 'Remote').length;
  const notCheckedInCount = Math.max(0, totalEmployeesCount - (presentCount + absentCount + lateCount + halfDayCount + onLeaveCount + wfhCount));
  const checkedOutCount = todayRecords.filter(a => a.checkOut && a.checkOut !== 'Still Working' && a.checkOut !== '-').length;
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await updateAttendanceRecord(editRecord.id, editStatus, editCheckIn, editCheckOut, editReason);
      setEditRecord(null);
      setEditReason('');
    } catch (err) {
  ...
  const handleProcessCorrection = async () => {
    ...
    try {
      if (correctionActionType === 'approve') {
        await approveAttendanceCorrection(processCorrectionModal.id, correctionReviewNote || 'Approved by Admin');
      } else if (correctionActionType === 'reject') {
        await rejectAttendanceCorrection(processCorrectionModal.id, correctionReviewNote || 'Insufficient proof provided');
      } else if (correctionActionType === 'request_info') {
        await requestCorrectionInfo(processCorrectionModal.id, correctionReviewNote || 'Please provide additional transit or medical proof.');
      }
                    ...
                    <tr>
                      <td colSpan="8" className="py-12 text-center text-rose-600 font-medium">
                        <div className="flex flex-col items-center justify-center gap-1">
                          <span className="font-extrabold">Failed to load attendance records</span>
                          <span className="text-xs text-rose-500">{attendanceError}</span>
                        </div>
                      </td>
                    </tr>
      ...
      {activeTab === 'corrections' && (
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {correctionsLoading ? (
              <div className="py-12 text-center text-slate-500 font-medium bg-white rounded-3xl border border-slate-200/80">
                <div className="flex items-center justify-center gap-2">
                  <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-saath-600"></div>
                  <span>Loading correction requests from database...</span>
                </div>
              </div>
            ) : correctionsError ? (
              <div className="py-12 text-center text-rose-600 font-medium bg-rose-50 rounded-3xl border border-rose-200">
                <p className="font-bold">Failed to load attendance corrections</p>
                <p className="text-xs text-rose-500 mt-1">{correctionsError}</p>
              </div>
            ) : attendanceCorrections.length === 0 ? (
              <div className="py-12 text-center text-slate-400 font-medium bg-white rounded-3xl border border-slate-200/80 p-8">
                <AlertCircle className="h-10 w-10 mx-auto text-slate-300 mb-2" />
                <p className="text-sm font-bold text-slate-700">No attendance correction requests found</p>
                <p className="text-xs text-slate-500 mt-1">Pending correction requests submitted by employees will appear here for administrative review.</p>
              </div>
            ) : (
